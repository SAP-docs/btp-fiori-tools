...
When adding local annotations, it is important to define them in the local annotation file of your project. Changes made to local copies of back-end annotation files and metadata files do not affect the deployed application. To change the annotation defined in the back end, copy the annotation along with the target and qualifier to the local annotation file and adjust it there. For more information, see [Overriding Annotations](overriding-annotations-2f1bb9c.md). Note that you cannot change annotations for value help if they are based on the additional metadata files for the value list service. These additional metadata files are referenced in the main metadata file using the `@Common.ValueListReferences` annotation.
